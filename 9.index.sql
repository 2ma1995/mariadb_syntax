...
-- id에 index가 걸려있으므로, id의 index를 통해 조회후 name full scan
select * from author where id = 1 and name ='hongildong';

-- 만약에 두컬럼에 동시에 index가 걸려있다면 해당 index찾음
-- 만약에 두컬럼 각각 index가 설정돼 있다면 mariadb엔진에서 최적의 알고리즘 실행
select * from author where name = 'hongildong' and address='서울시'

-- index제거
alter table author drpo index 인덱스명;
